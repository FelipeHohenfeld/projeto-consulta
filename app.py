...
num_processo= '0192538-15.2008.8.05.0001'

# entrar no site da - https://pje-consulta-publica.tjmg.jus.br/
driver = webdriver.Chrome()
driver.get('https://consultapublicapje.tjba.jus.br/pje/ConsultaPublica/listView.seam')
sleep(30)
# digitar número processo 
campo_processo = driver.find_element(By.XPATH,"//input[@id='fPP:numProcesso-inputNumeroProcessoDecoration:numProcesso-inputNumeroProcesso']")
campo_processo.send_keys(num_processo)
# clicar em pesquisar
botao_pesquisar = driver.find_element(By.XPATH,"//input[@class='btn btn-primary']")
...
for processo in processos:
    ...
    try:
        ...
        for index, linha in enumerate(pagina_processo.iter_rows(min_row=2,max_row=len(lista_movimentacoes),min_col=3,max_col=3)):
            for celula in linha:
                celula.value = str(lista_movimentacoes)[index]
        workbook.save('dados.xlsx')
        driver.close()
        sleep(5)
        driver.switch_to.window(driver.window_handles[0])
